...
#pragma region Graph
	...
	bool Graph::RemoveConnection(Connection const* ConnectionToRemove)
	{
		...
		AmountRemoved += std::erase_if(Connections,
		                               [&](std::unique_ptr<Connection> const& Element)
		                               {
			                               return *Element.get() == *ConnectionToRemove;
		                               });
		if (!bIsDirectional)
		{
			// Remove the inverse
			AmountRemoved += std::erase_if(Connections,
			                               [&](std::unique_ptr<Connection> const& Element)
			                               {
				                               return *Element.get() == InverseConnection;
			                               });
		}
	...
	void Graph::SetConnectionCostsToDistances()
	{
		for (auto& Connection : Connections)
		{
			FVector2D BetweenNodes{
				GetNode(Connection->GetFromId())->GetPosition() - GetNode(Connection->GetToId())->GetPosition()
			};
